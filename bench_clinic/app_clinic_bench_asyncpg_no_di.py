"""То же приложение, что app_clinic_bench_asyncpg.py, но БЕЗ FastAPI DI.

Провайдер-функции (get_state/get_settings/get_pool/get_connection/репозитории/сервисы/
get_current_user) остаются без изменений, но хендлер GET /surgery-offer вызывает их
ВРУЧНУЮ, сам прокидывая аргументы, вместо резолвинга графа через Depends(). Тела
провайдеров те же — исчезает только работа резолвера зависимостей FastAPI. Это
изолирует стоимость самого DI: сравнивая RPS этой версии против app_clinic_bench_asyncpg,
получаем чистый оверхед Depends() на запрос.
...
async def get_surgery_offer(
    ...
    async with get_connection(pool) as conn:
        user_repo = await get_user_repository(conn)
        patient_repo = await get_patient_repository(conn)
        procedure_repo = await get_procedure_repository(conn)
        medical_repo = await get_medical_repository(conn)
        schedule_repo = await get_schedule_repository(conn)
        billing_repo = await get_billing_repository(conn)
        surgery_repo = await get_surgery_repository(conn)
        pricing_service = await get_pricing_service(settings, billing_repo, surgery_repo)
        offer_service = await get_surgery_offer_service(
            settings, patient_repo, procedure_repo, medical_repo, schedule_repo, pricing_service
        )
        x_user_id = int(request.headers["X-User-Id"])
