"""Бенчмарк-вариант №2: SQLAlchemy + АСИНХРОННЫЕ зависимости.

То же, что app_clinic_bench_sync.py, но все провайдеры объявлены `async def` → выполняются
на event-loop без диспетчеризации в threadpool. Единственная разница с sync-файлом — ключевые
слова у провайдеров-зависимостей.
...
async def get_session_factory(state: Annotated[State, Depends(get_state)]) -> async_sessionmaker:
...
async def get_patient_repository(session: Annotated[AsyncSession, Depends(get_session)]) -> PatientRepository:
    return PatientRepository(session)


async def get_procedure_repository(session: Annotated[AsyncSession, Depends(get_session)]) -> ProcedureRepository:
    return ProcedureRepository(session)


async def get_medical_repository(session: Annotated[AsyncSession, Depends(get_session)]) -> MedicalRecordRepository:
    return MedicalRecordRepository(session)


async def get_schedule_repository(session: Annotated[AsyncSession, Depends(get_session)]) -> ScheduleRepository:
    return ScheduleRepository(session)


async def get_billing_repository(session: Annotated[AsyncSession, Depends(get_session)]) -> BillingRepository:
    return BillingRepository(session)


async def get_surgery_repository(session: Annotated[AsyncSession, Depends(get_session)]) -> SurgeryRepository:
...
app = FastAPI(
    title="Клиника пластической хирургии «До и После» — bench async deps",
