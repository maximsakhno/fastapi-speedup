"""Та же ручка и бизнес-логика, что app_clinic_bench_asyncpg_no_di.py, но веб-слой —
голый Starlette вместо FastAPI.

Бизнес-классы, репозитории, сервисы и pydantic-модели идентичны no-DI-версии; ответ по-прежнему
сериализуется pydantic-ом (result.model_dump_json()). Меняется только веб-слой: роутинг Starlette,
ручной парсинг query/headers, сборка графа прямо в хендлере, отдача готового Response. Сравнивая RPS
против app_clinic_bench_asyncpg_no_di, получаем стоимость слоя FastAPI поверх Starlette.
...
async def surgery_offer(request: Request) -> Response:
    params = SurgeryOfferParams(
        patient_id=request.query_params["patient_id"],
        procedure_code=request.query_params["procedure_code"],
        preferred_date=request.query_params["preferred_date"],
...
# 4xx как JSON {"detail": ...} — зеркалим поведение FastAPI (у голого Starlette дефолт — plain text).
async def http_exception_handler(request: Request, exc: HTTPException) -> JSONResponse:
    return JSONResponse({"detail": exc.detail}, status_code=exc.status_code)


app = Starlette(
    routes=[Route("/surgery-offer", surgery_offer, methods=["GET"])],
    lifespan=lifespan,
